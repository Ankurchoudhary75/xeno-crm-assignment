...
const CRM_RECEIPT_URL = process.env.CRM_RECEIPT_URL || 'http://localhost:3000/api/crm/receipt';
...
export async function POST(req: Request) {
  try {
    ...
    const response = NextResponse.json({ status: 'Accepted', communicationId }, { status: 202 });
    ...
    return NextResponse.json({ error: 'Failed to accept message' }, { status: 500 });
  }
}
