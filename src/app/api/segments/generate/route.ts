...
Your task is to convert a user's natural language request into a valid SQLite SQL query to fetch matching customers.
...
Example Output: SELECT DISTINCT c.* FROM Customer c JOIN "Order" o ON c.id = o.customerId GROUP BY c.id HAVING SUM(o.amount) > 50`;
    ...
    return NextResponse.json({ error: error.message || 'Failed to generate segment' }, { status: 500 });
  }
}
