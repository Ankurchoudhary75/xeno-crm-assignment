...
export default async function CampaignsPage() {
  ...
  const runningCount = campaigns.filter((c) => {
    const hasPending = c.communications.some(
      (comm) =>
        comm.status === "PENDING" ||
        comm.status === "SENT" ||
  ...
  return (
    <div className="flex-col gap-8">
      ...
      {campaigns.length === 0 ? (
        <div
          className="card flex-col items-center justify-center gap-4 py-12"
          style={{ opacity: 0.6 }}
        >
        ...
        >
          {campaigns.map((camp) => {
            ...
            camp.communications.forEach((c) => {
              statusCount[c.status as keyof typeof statusCount]++;
            });

            const isRunning =
              camp.communications.some(
                (c) =>
                  c.status === "PENDING" ||
                  c.status === "SENT" ||
                  c.status === "DELIVERED",
            ...
            const channelIcon =
              camp.channel === "WHATSAPP" ? (
                ...
                <MessageSquare size={16} color="var(--accent-amber)" />
              ) : camp.channel === "EMAIL" ? (
            ...
            return (
              ...
              >
                ...
                >
                  <StatChip
                    label="Delivered"
                    pct={getPct(statusCount.DELIVERED)}
                    color="var(--accent-emerald)"
                  />
                  <StatChip
                    label="Opened"
                    pct={getPct(statusCount.OPENED)}
                    color="var(--accent-amber)"
                  />
                  <StatChip
                    label="Clicked"
                    pct={getPct(statusCount.CLICKED)}
                    color="var(--accent-indigo)"
                  />
                  <StatChip
                    label="Failed"
                    pct={getPct(statusCount.FAILED)}
                ...
                >
                  ...
                  {statusCount.DELIVERED > 0 && (
                    <div
                      style={{
                        width: `${(statusCount.DELIVERED / total) * 100}%`,
                        background: "var(--accent-emerald)",
                      }}
                    />
                  )}
                  {statusCount.OPENED > 0 && (
                    <div
                      style={{
                        width: `${(statusCount.OPENED / total) * 100}%`,
                        background: "var(--accent-amber)",
                      }}
                    />
                  )}
                  {statusCount.CLICKED > 0 && (
                    <div
                      style={{
                        width: `${(statusCount.CLICKED / total) * 100}%`,
                  ...
                  {statusCount.FAILED > 0 && (
                    <div
                      style={{
                        width: `${(statusCount.FAILED / total) * 100}%`,
